...
export default function HeroSection() {
  ...
  // Captura o scroll da página para criar o movimento vertical
  const { scrollY } = useScroll();
  
  // Transformações para o efeito Parallax (movimento vertical ao rolar)
  const yAura = useTransform(scrollY, [0, 1000], [0, 200]);
  const yContent = useTransform(scrollY, [0, 1000], [0, -50]);
  const yMockup = useTransform(scrollY, [0, 1000], [0, -120]);
  ...
  return (
    <section className="relative min-h-[95vh] flex items-center justify-center overflow-hidden bg-background py-12 md:py-20">
      ...
      <motion.div style={{ y: yAura }} className="absolute inset-0 z-0 overflow-hidden pointer-events-none">
      ...
      <div className="container relative z-20 mx-auto px-4 w-full">
        {/* Aplica um leve movimento de subida no conteúdo principal ao rolar */}
        <motion.div style={{ y: yContent }} className="max-w-7xl mx-auto flex flex-col items-center gap-16 md:gap-20 text-center">
          
          {/* CONTEÚDO TEXTUAL COM ANIMAÇÃO DE ENTRADA */}
          <motion.div 
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.8, ease: "easeOut" }}
            className="flex flex-col items-center text-center space-y-6 md:space-y-10 w-full max-w-5xl"
          >
            ...
            {/* DIFERENCIAIS COM EFEITO DE SURGIMENTO AO ROLAR */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6 w-full max-w-4xl px-2">
              {features.map((item, i) => (
                <motion.div 
                  ...
                  className="glass-card p-5 rounded-2xl group flex items-start gap-5 hover:border-primary/50 hover:-translate-y-1 hover:shadow-2xl transition-all duration-300 border border-white/5"
                >
          ...
          {/* ÁREA VISUAL COM PARALLAX ACENTUADO */}
          <motion.div 
            style={{ y: yMockup }}
            className="relative group lg:w-max lg:h-max w-full max-w-[320px] md:max-w-2xl px-4"
          >
